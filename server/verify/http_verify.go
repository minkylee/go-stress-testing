...
func HTTPResponseCodeVerify(request *model.Request, response *http.Response) (code int, isSucceed bool) {
	defer func() {
		_ = response.Body.Close()
	}()
	if body, err := getZipData(response); err != nil {
		code = 502
		isSucceed = false
	} else {
		var ret = struct {
