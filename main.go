...
var (
	...
	random             = "false" // 通过文件传入多个请求时，是否随机选取请求
...
func init() {
	...
	flag.StringVar(&random, "r", debugStr, "调试模式")
...
func main() {
	...
	requests, err := model.NewRequest(requestURL, verify, 0, debug, path, headers, body)
	if err != nil {
		fmt.Printf("参数不合法 %v \n", err)
		return
	}
	fmt.Printf("\n 启动测试	client并发数: %d	单client请求数: %d\n", concurrency, totalNumber)
	// 开始处理
	server.Dispose(concurrency, totalNumber, requests, random == "true")
	return
}
