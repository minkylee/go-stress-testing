...
func init() {
	...
	model.RegisterVerifyHTTP("entry_task", verify.HTTPResponseCodeVerify)
...
// Dispose 处理函数
func Dispose(concurrency, totalNumber uint64, requests []*model.Request, randomRequest bool) {
	if len(requests) < 1 {
		fmt.Println("没有请求")
		return
	}
