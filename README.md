...
声明：本项目源自Github的开源项目 [go-stress-testing](https://github.com/link1st/go-stress-testing)，本人加了一项小功能

go build mian.go得到可执行文件后

```
./go-stress-testing -p /test.txt -n 1 -c 200 -v entry_task -r true
```

- -p：指定的curl文件，可以包含多行请求的curl，-c指定的client会使用这些curl，相当于每个client都会使用不同的curl

  - curl示例

    ```
    curl --location --request POST 'http://127.0.0.1:8080/user_login' --header 'Cookie: sessionId=FpHnp3z33qK0RSAkQj30U5KB5IDiD1JC' --form 'username=test_user' --form 'password=12345678'
    ```

- -r: 指定client是否随机使用文件中的curl，false：client顺序使用文件中的curl，true：client随机使用文件中的curl
